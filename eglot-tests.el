...
(ert-deftest eglot-server-programs-simple-executable ()
  (dolist (executable-name (list "some-executable" eglot-missing-executable-name))
    (let ((eglot-server-programs `((foo-mode ,executable-name)))
          (major-mode 'foo-mode))
      (eglot--guessing-contact (interactive-p prompt-args guessed-class guessed-contact)
        (if (equal executable-name eglot-missing-executable-name)
            (should (equal (not prompt-args) (not interactive-p)))
          (should (not prompt-args)))
        (should (equal guessed-class 'eglot-lsp-server))
        (should (equal guessed-contact (list executable-name)))))))
...
(ert-deftest eglot-server-programs-executable-multiple-major-modes ()
  (dolist (executable-name (list "some-executable" eglot-missing-executable-name))
    (let ((eglot-server-programs `(((bar-mode foo-mode) ,executable-name)))
          (major-mode 'foo-mode))
      (eglot--guessing-contact (interactive-p prompt-args guessed-class guessed-contact)
        (if (equal executable-name eglot-missing-executable-name)
            (should (equal (not prompt-args) (not interactive-p)))
...
(ert-deftest eglot-server-programs-executable-with-arg ()
  (dolist (executable-name (list "some-executable" eglot-missing-executable-name))
    (let ((eglot-server-programs `((foo-mode ,executable-name "arg1")))
          (major-mode 'foo-mode))
      (eglot--guessing-contact (interactive-p prompt-args guessed-class guessed-contact)
        (if (equal executable-name eglot-missing-executable-name)
            (should (equal (not prompt-args) (not interactive-p)))
          (should (not prompt-args)))
        (should (equal guessed-class 'eglot-lsp-server))
        (should (equal guessed-contact (list executable-name "arg1")))))))
...
(ert-deftest eglot-server-programs-class-name-and-contact-spec ()
  (dolist (executable-name (list "some-executable" eglot-missing-executable-name))
    (let ((eglot-server-programs `((foo-mode bar-class ,executable-name "arg1"
                                             :autoport "arg2")))
          (major-mode 'foo-mode))
      (eglot--guessing-contact (interactive-p prompt-args guessed-class guessed-contact)
        (if (equal executable-name eglot-missing-executable-name)
